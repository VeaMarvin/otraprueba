        ...
        @if (!is_null($pedidos_creditos))
            ...
            <table class="datagrid" style="width: 100%">
                ...
                @foreach($pedidos_creditos as $item)
                    ...
                    <td width="100" style="text-align:right;">{{ $item->getStringTotalAttribute() }}</td>
                    <td width="80" style="text-align:center;">{{ $item->getStringFechaAttribute() }}</td>
                    <td width="80" style="text-align:right;">{{ $item->status }}</td>
                    </tr>
                @endforeach
                </tbody>
        ...
        @if (!is_null($pagar))
            ...
            <table class="datagrid" style="width: 100%">
                ...
                @foreach($pagar as $item)
                    <tr>
                    <td width="40" style="text-align:center;">#{{ $item->order_id }}</td>
                    <td width="60" style="text-align:left;">{{ $item->getUsuarioAttribute() }}</td>
                    <td width="60" style="text-align:center;">{{ $item->credit->getCreditConcatAttribute() }}</td>
                    <td width="60" style="text-align:right;">{{ $item->getTotalConcatAttribute() }}</td>
                    <td width="60" style="text-align:center;">{{ $item->getDateIAttribute().' - '.$item->getDateFAttribute() }}</td>
        ...
        @if (!is_null($creditos))
            ...
            <table class="datagrid" style="width: 100%">
                <thead>
                    <tr>
                    <th style="text-align:center;" scope="col">{{ __('Cliente') }}</th>
                    <th style="text-align:center;" scope="col">{{ __('Aplicando al crédito') }}</th>
                    <th style="text-align:center;" scope="col">{{ __('Empleado respondio') }}</th>
                    <th style="text-align:center;" scope="col">{{ __('Respuesta') }}</th>
                    <th style="text-align:center;" scope="col">{{ __('En uso') }}</th>
                    </tr>
                </thead>
                <tbody>
                @foreach($creditos as $item)
                    <tr>
                    <td width="125" style="text-align:left;">{{ $item->getUsuarioAttribute() }}</td>
                    <td width="80" style="text-align:center;">{{ $item->getCreditConcatAttribute() }}</td>
                    <td width="125" style="text-align:left;">{{ $item->getEmpleadoAttribute() }}</td>
                    <td width="80" style="text-align:right;">{{ "Aprobación: $item->approved" }}</td>
                    <td width="60" style="text-align:center;">{{ $item->getUsoAttribute() }}</td>
                    </tr>
                @endforeach
                </tbody>
                ...
                <tfoot>
                <tr>
                    <th style="text-align:left; background: grey;" colspan="5" scope="col">{{ $footer }}</th>
                </tr>
                </tfoot>
            </table>
        @endif

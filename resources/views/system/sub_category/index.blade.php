...
@section('content')
  <div class="content">
    <div class="container-fluid">
            ...
            <div class="col-md-12">
                <nav aria-label="breadcrumb" role="navigation">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Sub categorías de {{ $category->name }}</li>
                    </ol>
                </nav>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                <form method="post" action="{{ route('sub_category.store') }}" autocomplete="off" class="form-horizontal">
                    @csrf
                    @method('post')
                    <div class="card ">
                        <div class="card-header card-header-success">
                            <h4 class="card-title">{{ __('Nueva sub categoría') }}</h4>
                            <p class="card-category">{{ __("Registrar una nueva sub categoría en el sistema para la categoría {$category->name}") }}</p>
                        </div>
                            ...
                            <div class="row">
                                <div class="col-sm-12 col-md-12">
                                    ...
                                    <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}"
                                        name="name" id="input-name" type="text" placeholder="{{ __('sub categoría') }}" value="{{ old('name') }}" aria-required="true"/>
                                    @if ($errors->has('name'))
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header card-header-info">
                        <h4 class="card-title ">{{ __("Sub categorías de {$category->name}") }}</h4>
                        <p class="card-category"> {{ __("En esta pantalla el sistema muestra todas las sub categorías registradas de la categoría {$category->name}.") }}</p>
                    </div>
                    <div class="card-body">
                        <form method="get" action="{{ route('category.show', $category) }}" autocomplete="off" class="form-horizontal">
                            @csrf
                            @method('get')
                                    ...
                                    @foreach($items as $item)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <form method="post" action="{{ route('sub_category.delete', $item) }}">
                                                    @csrf
                                                    @method('delete')
                                                    <a rel="tooltip" class="btn btn-warning btn-sm btn-round" href="{{ route('sub_category.edit', $item) }}" data-toggle="tooltip" data-placement="top" title="Editar información">
